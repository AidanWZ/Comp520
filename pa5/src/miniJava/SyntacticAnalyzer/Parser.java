...
public class Parser {
	...
	public Identifier parseIdentifier() throws SyntaxError {
		...
		if (currentToken.kind == Token.IDENTIFIER) {			
			...
			temp = new Token(Token.IDENTIFIER, currentToken.spelling, currentToken.position);
			accept(Token.THIS, "parseIdentifier");
			previousTokenPosition = currentToken.position;
			return new Identifier(temp);
		}
		else {
	...
	public Statement parseStatement() throws SyntaxError{
		...
		switch(currentToken.kind){
			...
			case Token.STRING: 
				...
				id_string = parseIdentifier();					
				if (currentToken.kind == Token.EQUALS) {
					accept(Token.EQUALS, "parseStatement");
					e_int = parseExpression();
					accept(Token.SEMICOLON, "parseStatement");
					return new VarDeclStmt(new VarDecl(new BaseType(TypeKind.STRING, current.position), id_string.spelling, current.position), e_int, current.position);
				}		
				else if (currentToken.kind == Token.SEMICOLON) {
					accept(Token.SEMICOLON, "parseStatement");
					return new VarDeclStmt(new VarDecl(new BaseType(TypeKind.STRING, current.position), id_string.spelling, current.position), null, current.position);
				}		
				else {
					return null;
				}					
	...
	public Reference parseReference() throws SyntaxError {
		...
		Reference ref = null;
		//get first reference of the sequence
		if (current.kind == Token.THIS) {
			ref = new ThisRef(current.position);
		}
		else if (current.kind == Token.IDENTIFIER) {
			ref = new IdRef(id, current.position);
		}
		//if qualified get qualifiers
		if (currentToken.kind == Token.PERIOD) {
			while(currentToken.kind == Token.PERIOD) {
				acceptIt();
				if (first) {
					this.refer = new QualRef(ref, parseIdentifier(), current.position);
					first = false;
				}
				else {
					this.refer = new QualRef(this.refer, parseIdentifier(), current.position);
				}				
			}
			first = true;
			return this.refer;
		}
		//if not qualified return simple reference
		else {
			if (current.kind == Token.THIS) {
