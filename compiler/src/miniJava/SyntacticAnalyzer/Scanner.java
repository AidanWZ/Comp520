...
public final class Scanner {
	...
	private int scanSeparator() {
		...
		case '/': {
			...
			switch(currentChar) {
				case '/': {
					// Has to be a do while because it needs to execute takeIt() even if the character is newLine or null character
					do takeIt();
					while(currentChar != '\n' && currentChar != '\u0000');
				}
				return 2;
				case '*': {
					...
					while (true){
						...
						// if the next character is a * then it is an empty comment "/**/"
						if(currentChar != '*'){
							takeIt();
							continue;
						}
						takeIt();
						// This only occurs at the end of the comment
						if(currentChar == '/'){
							takeIt();
							break;
						}		
		...
		// Syntax is the same for all of these cases
		case '\n': case ' ': case '\t': case '\r': {
			do{
				takeIt();
			}
